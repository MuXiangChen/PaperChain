...
- [Standards System（学科规范系统）](#standards-system-discipline-packs)
- [Canon Analyzer（范文分析模块）](#canon-analyzer范文分析模块)
- [IdeaGraph Engine（研究思路图引擎）](#ideagraph-engine研究思路图引擎)
- [Idea Workspace & Evidence Builder（证据构建模块）](#idea-workspace--evidence-builder证据构建模块)
- [Outline Weaving Engine（大纲编织引擎）](#outline-weaving-engine大纲编织引擎)
- [Editing & Refinement Engine（编辑与优化引擎）](#editing--refinement-engine编辑与优化引擎)
- [Assist Plugins（辅助插件系统）](#assist-plugins辅助插件系统)
- [FigureLab（图表生成模块）](#figurelab图表与结构图生成模块)
- [Export Renderers（文档导出模块）](#export-renderers文档导出模块)
